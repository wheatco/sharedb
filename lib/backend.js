...
Backend.prototype.subscribe = function(agent, index, id, version, callback) {
  ...
  var closeHandler = function() {
    backend.db.getSnapshot(collection, id, null, null, function(err, snapshot) {
      if (err) return;
      var request = {
        agent: agent,
        index: index,
      ...
      backend.trigger('unsubscribe', agent, request, function(err) {
        // unsubscribe errors are ignored
      });
    });
  }
  backend.db.getSnapshot(collection, id, null, null, function(err, snapshot) {
    ...
    backend.trigger('subscribe', agent, request, function(err) {
      if (err) return callback(err);
