  ...
  if (op.op) {
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        this.emit('confirmed op', op.op, op.v)
      }
    ...
    // submit a 'confirmed op' event, but only if this op came from the server (and is therefore acked)
    if (!source) {
      this.emit('confirmed op', op.op, op.v)
    }
    return;
...
Doc.prototype._opAcknowledged = function(message) {
  ...
  this.emit('confirmed op', this.inflightOp.op, message.v)
